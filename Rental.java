...
Rental implements Serializable {
    ...
    public void setActualDateReturned
            (GregorianCalendar actualDateReturned) {
        this.actualDateReturned = actualDateReturned;
    }
    ...
    /******************************************************************
     * Method to compare two rentable objects, which is either game or
     * console, then due date. And finally, the renter's name.
     *
     * @param s - represents a Rental object
     * @return -1 if this comes before rental
     *          1 if this comes after rental
     *          0 if this is equal to rental
     *****************************************************************/
    public int compareTo(Rental s) {
        if (this instanceof Game && s instanceof Console)
            return -1;

        if (this instanceof Console && s instanceof Game)
            return -1;

        if (this.dueBack.equals(s.dueBack))
            return this.nameOfRenter.compareToIgnoreCase
                    (s.nameOfRenter);

        return this.dueBack.compareTo(s.dueBack);
    }
}